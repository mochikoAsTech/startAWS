= ELBとAuto Scalingで落ちても自動復旧するサイトにしよう
...
皆さんが本著で構築した環境は「EC2のウェブサーバ1台＋RDSのデータベースサーバ1台」という低コストのお手軽構成ですので、ウェブサーバもデータベースサーバもどちらもSPOFになっています。たとえばAMIでEBSボリュームのEBSスナップショットを取る間、EC2のインスタンスを停止したら途端にWordPressのサイトは見られなくなります。
