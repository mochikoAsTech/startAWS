...
AWSのユーザにはクレジットカードを紐付けているのですから、お財布もルートユーザーも、等しく扱いには気をつけなければいけません。たとえば悪い人があなたのルートユーザのEメールアドレスとパスワードを盗んで、こっそりマネジメントコンソールにサインインしたとします。ルートユーザーならなんでもできるので、景気良くいちばん高いサーバ@<fn>{d28xlarge}を100台立てたとしましょう。その場合、1日で180万、1ヶ月で5,400万の請求があなたのところへやってきます。（@<img>{awsUnauthorizedUse}）
...
本著では自分ひとりだけで作業をする想定なので、IAMユーザーも1人しか作りません。ですがもし業務などで複数名でマネジメントコンソールにサインインするような場合は、IAMユーザーは必ず1人につき1つずつ作成してください。まったく同じ作業をするから開発チーム内のAさんとBさんは同じIAMユーザを共有すればいいのでは？と思われる場合でも、必ずAさんBさんそれぞれに別々のIAMユーザーを用意することをお勧めします。
...
//footnote[administratorAccess][え、大金の詰まったアタッシュケース持って買い物に行っちゃうの？！と思ったあなたは正しいです。ですが、ここでは「本来は必要最小限の権限だけを付与すべき」ということだけ覚えておいて先へ進みましょう。]
...
IAMのグループが作成できたので、続いてIAMユーザーを作成しましょう。左メニューから「ユーザー」を選んでください。ユーザーの一覧画面（@<img>{selectUser}）が表示されます。まだIAMユーザーが1つも存在しないため、「IAMユーザーが存在しません。」と表示されています。それでは左上の青い「ユーザーを追加」をクリックしてください。
...
先ずはステップ1です。IAMのユーザー名を入力してください。本著ではIAMのユーザ名は「start-aws-user」にします。IAMのユーザー名はこの後もサインイン時に使用しますので、もし別のユーザー名にした場合は、しっかりメモしておいてください。続いてこのIAMユーザーでAWSにアクセスする方法を選択します。たとえばAWSで「サーバを立てる」「サーバを削除する」などの操作をするには、
...
ユーザー名を入力して、アクセスの種類を選択したら「次のステップ: アクセス権限」をクリックしてください。

続いてステップ2です。ユーザーを先に作っておいた「start-dns-group」というグループに追加（@<img>{iamStep2}）します。「start-dns-group」にチェックを入れたら、「次のステップ: 確認」をクリックしてください。
...
しっかりメモしてメールも送ったら、「https://************.signin.aws.amazon.com/console」と書いてあるURLをクリックします。するとブラウザの別タブでIAMユーザーのサインイン画面が開いて、自動的に元のタブのルートユーザーはサインアウト（@<img>{signOut}）させられます。こちらのタブは閉じてしまって構いません。
...
IAMユーザーでサインインしてマネジメントコンソールが表示されたら、再び左上にある「サービス」から「IAM」をクリックしてください。もしくは「最近アクセスしたサービス」から「IAM」をクリック（@<img>{accessService}）でも構いません。
