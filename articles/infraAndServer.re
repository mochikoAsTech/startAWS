...
「ラックマウント型サーバ」だけでなく、デスクトップパソコンのような形状の「タワー型サーバ」（@<img>{towerAndBlade}）@<fn>{tower}や、シャーシやエンクロージャーと呼ばれる箱の中に何本も差し込んで使う省スペースな「ブレードサーバ」@<fn>{blade}など、サーバには色々な形があります。
...
そしてこうしたラックマウント型サーバ、タワー型サーバ、ブレードサーバのように、手で触れる実体があるサーバのことを@<ttb>{物理サーバ}といいます。物理的な実態があるから物理サーバです。（この「物理サーバ」という言葉は後でまた出てきますので覚えておいてください）
 ...
 * 万が一完全に電力が途絶えたらUPS（無停電電源装置）が自動起動
 * さらにUPSが動いている間に自家発電機が稼動し、最低数日間は追加の燃料給油なしで稼動可能
...
ところで部屋を借りるとき、「1DKの部屋なら12万、2LDKの部屋なら20万」のように広いほうが家賃は高くなります。データセンターを借りるときもまったく同じで「ラックの1/2なら12万、1ラックまるごとなら20万」のように、ラックサイズによって月額の費用が変わってきます。

そして2LDKの部屋に1人で住むと、1人で20万負担しなければなりませんが、シェアハウスにして10人で住めば1人当たりの家賃コストは2万円に下がります。サーバラックも同様で、42Uのラックに1Uサーバを1台しか乗せないより、42台詰め込んだ方が1台当たりのラック使用コストは下がります。
...
一軒家には基本的に1世帯しか住めません@<fn>{house}が、マンションにすれば土地のサイズは同じままで10世帯住むことができます。1台の物理サーバをそのまま使うのではなく、物理サーバ上に何台ものバーチャル（仮想的）なサーバを作ることで、サーバラックのサイズはそのままでサーバ台数を増やすことができたのです。
...
前述のとおり物理的な実体があるのが物理サーバですが、その逆で手で触れる物理的な実態がないのが仮想サーバです。手で触れられるのはあくまでホストOSのサーバであり、ゲストOSのサーバはその中で仮想的にしか存在しないため、手で触ることはできません。
...
「どのメーカーのサーバにしよう？EHPかな？それともIBMかな？DELLがいいかな？」と各社の見積もりをとり、値引き交渉をして、それでも数十万から数百万するので社内の稟議を通してやっと購入。購入してもすぐ届くわけではなく、数週間待ってやっと届きます。そして届いたらサーバを段ボールから出して、データセンターもしくは自社のサーバルームにあるサーバラックのところまで持って行って、がっちゃんこと設置。
...
但し、長い目で見るとフードコートにテナント料を払い続ける方が、土地や建物を買うよりも最終的には高くなるかも知れません。前述のとおり初期投資は少なくて済むのですが、クラウドのいいところは決して「コストが安くなる」ということではありません。実際、AWSは他社の共有レンタルサーバやVPS@<fn>{vps}と比べると高額です。

//footnote[vps][Virtual Private Serverの略。先ほど出てきた仮想サーバのことだと思ってください。]

クラウドのよいところは、前述のすぐに始められてすぐにやめられる初期コストの低さ。それからショッピングセンター内でフードコートが入っている南館が地震で崩れてしまっても、すぐに北館に移ってピザ屋の営業を再開できる、といった冗長性です。
...
そしてようやく最初の話に戻ると、AWSとはアマゾン ウェブ サービス（Amazon Web Services）の略で、欲しいものをぽちっとな！すると翌日には届くあのAmazonがやっているクラウドなのです。
...
こうしたプライベートクラウドだと「初期投資額が少ない」「サーバの性能や台数を後から好きなだけ増強できる」といった、クラウド本来のメリットが享受できないように思えますが、果たしてこれもクラウドなのでしょうか？
