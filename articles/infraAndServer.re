...
これだと建物はもうあって、電気ガス水道ももう用意されています。フードコート内の一区画を契約して使わせてもらうだけなので、すべて自分で準備するオンプレミスと違ってすぐに始められます。しかも数か月やってみて「もうピザ焼くの飽きたわー！」と思ったら、その区画を借りるのを止めるだけでいいのです。前述のとおりAWSなら使い始めるときの初期費用もなく1秒単位の従量課金なので「ピザ屋さんもうやめたいけど、この先30年のローン支払いが残ってるからやめるにやめられない・・」ということもありません。「前月の25日までに契約終了を申し出る必要がある」といった制限すらないので、本当にいつでもやめられます。@<fn>{foodcourt}

//footnote[foodcourt][とはいえ現実のフードコートで出店する場合は契約期間の縛りがあるでしょうし、止めようと思っても即日でやめられるものではないですよね。ちなみにEC2の料金体系には従量課金の「オンデマンド」の他に「リザーブドインスタンス」という長期利用が確約できれば料金が最大75%も安くなるプランがあります。つまり「僕と契約して低コストになってよ！」ということなので途中解約ができないリスクは認識した上で利用しましょう。]
...
この冗長性を自力で確保しようとしたら大変です。ピザ屋さんを常に営業し続けておくために、いつ来るか分からない火災に備えて最初から予備の店舗も確保しておかなければならないとしたら相当なコストがかかります。オンプレミスのサーバなら、ただ自社サイトを作りたいだけなのに、品川と渋谷の2か所でデータセンターを借りて両方に1台ずつサーバを用意しておき、もし品川のデータセンターが火災で使えなくなっても渋谷のデータセンターにあるサーバは生きているのでサイトは見られる、という構成にしておかなくてはなりません。予見できない災害に対して冗長性を確保するのは大仰な話@<fn>{dr}です。このような構成が簡単に実現できるクラウドはすごいですよね。
...
そしてようやく最初の話に戻ると、AWSとはAmazon Web Servicesの略で、欲しいものをぽちっとな！すると翌日には届くあのAmazonがやっているクラウドなのです。
