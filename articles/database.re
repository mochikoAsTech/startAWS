...
こちらをご覧ください！このおしゃれなブログはWordPressというCMSで動いています。

//comment{
TODO:後でWordPressのトップページ画像を入れる
//}
...
CMSにはWordPress（ワードプレス）とMovable Type（ムーバブル・タイプ）というツートップが存在してるのですが、どちらも個人利用であれば無料で使うことができます。今回はCMS界のトップシェアであるWordPressを使えるように環境を構築していきましょう。
...
データベースはOracle DatabaseやPostgreSQLなどMySQL以外にも色々な種類があるのですが、その中でMySQLはもっとも普及しているデータベースでシェアの8割以上を占めています。

MySQLは以前はSun Microsystemsという会社が所有してオープンソースで公開していましたが、2009年にSunがOracleに買収されたため現在はOracleのものになっています。Oracleに買収された後もありがたいことにMySQLは引き続きオープンソースで公開されており現在も無償で使えますが、OracleはもともとOracle Databaseという有償のデータベースも所有しているためMySQLはその競合にあたり、今は平気でもいつかはサポートが終了したりオープンソースで公開されなくなるのでは？という懸念もあります。
...
//footnote[apache][サーバにApacheをインストールしてウェブサーバを作ったのと同じように、MySQLをインストールしてデータベースサーバを作る、ということです。既にMySQLがインストールされたAMIからインスタンスを作る形でも構いません。]

Amazon RDSはAmazon Relational Database Serviceの略で、ざっくり言うとデータベースサーバです。データベースに接続することはできますが、MySQLが動いているサーバにSSHでログインすることはできません。RDSではデータベースエンジンとしてMySQLだけでなくAmazon Aurora@<fn>{aurora}、Microsoft SQL Server、Oracle、PostgreSQLなども選択可能です。

//footnote[aurora][本当は「MySQLの最大5倍のパフォーマンス」といわれるMySQL互換のデータベース、Amazon Auroraを使いたかったのですがAWSの無料利用枠に含まれないため断念しました。WordPressはAuroraでも動くようです。]
...
RDSのインスタンス作成に先駆けて、パラメータグループを作成します。パラメータグループが何なのかについては追って説明しますので、とりあえず右上にある「パラメータグループの作成」をクリック（@<img>{startaws84}）してください。
...
先ほど作ったパラメータグループというのはこのパラメータをまとめたグループのことなのですが、この後RDSでインスタンスを作るときに「どのパラメータグループを使用するか？」という選択肢が出てくるため、インスタンスを作る前にパラメータグループを作っておく必要があったのです。
...
//image[startaws87][「character_set」でパラメータを検索][scale=0.8]{
//}
...
//image[startaws88][5つのパラメータで値を「utf8mb4」にして「変更の保存」をクリック][scale=0.8]{
//}
...
難しそうなことが書いてありますが、要はこれらのパラメータを「utf8mb4」に設定しておくと日本語や絵文字が文字化けしなくなる、という効能だけ覚えておいてください。逆に言えばこれらを設定しないとWordPressの記事で日本語や絵文字を使ったときに文字化けしてしまいます。それでは右上にある「変更の保存」をクリックしてください。

ちなみに次の2つは似たような名前なのですがこちらは何も設定しなくて大丈夫です。

 * character_set_filesystem : MySQL Serverがファイルシステムにアクセスする際、ファイル名の解釈に用いられる文字コード。デフォルト設定のままでよいため設定不要。
 * character_set_system : テーブルやカラムなどのオブジェクト名に対して用いられる文字コードで値は常にutf8。そもそもパラメータが存在しないため設定不要。
...
//image[startaws91][「collation_connection」の値を「utf8mb4_general_ci」にして「変更の保存」をクリック][scale=0.8]{
//}
...
これでパラメータグループの作成は完了です。続いてオプショングループも作成します。
...
//image[startaws99][MySQLを選択したら「次へ」をクリック][scale=0.8]{
//}

=== ステップ 2:DB 詳細の指定

「インスタンスの仕様」は何も変更せずそのままで構いません。
...
ここで設定した「マスターユーザの名前」と「マスターパスワード」は、この後でWordPressがデータベースに接続するときに必要となります。しっかり覚えておいてください。入力したら「次へ」をクリックします。
...
ここで設定した「データベースの名前」も、この後でWordPressがデータベースに接続するときに必要となります。こちらもしっかり覚えておいてください。「データベースの設定」のそれ以外の項目や、さらに下にある「暗号化」「バックアップ」「モニタリング」「ログのエクスポート」「メンテナンス」@<fn>{maintenance}はすべてデフォルトのままで構いません。
...
データベースに記事データを保存したり、データベースへ記事データを取りに行ったりするのはEC2のインスタンス上で動いているWordPressですので、このセキュリティグループではEC2のインスタンスから「MySQL（ポート番号3306番）」へ通信できるように許可する設定をしてやらなければいけません。
...
これでEC2のインスタンスからのみRDSのインスタンスのMySQLへ接続できるようになりました。
...
//image[startaws110][左メニューの「インスタンス」をクリック][scale=0.8]{
//}
...
パスワードを入力後、次のように最後に「mysql> 」と表示されたらMySQLへのログインに成功しています。
