...
個人的には、儀同世津子がパソコンのスイッチをつけて「立ち上がろうとしてるところ」と言ったら、瀬戸千衣は「立ち上がるの？」と眉を寄せた@<fn>{whoInside}ので私は「立てる」派です。
...
SSHは「サーバに入れて」とリクエストされたら、その人が誰なのか確認した上でサーバに入れてくれます。つまりSSHサーバは、サーバへアクセスしてきた客に対して@<ttb>{「ネットワークを介してサーバにログインできる」というサービスを提供}しているのです。SSHがあることによってデータセンターにあるサーバと自分が使っている目の前のパソコンをセキュアに繋ぐことができるのです。
...
続いてターミナルで次の文字を入力したら再びReturnキーを押します。「IPv4パブリックIP」の部分は先ほどメモした「IPv4パブリックIP」に書き換えてください。-iオプションは「サーバにはこの鍵を使って入ります」という意味ですので、もし「start-aws-keypair.pem」を保存した場所がデスクトップ@<fn>{tilde}以外の場合は適宜書き換えてください。
...
実はEC2においてIPアドレスは動的なものです。つまりインスタンスが停止されると、そのインスタンスに紐づいていたIPアドレスは解放されてしまい、インスタンスが再起動されるとまた新しいIPアドレスが紐づけられる、というのがEC2におけるIPアドレスの仕様@<fn>{reboot}なのです。会社にたとえると、毎日19時に仕事を終えてオフィスのブレーカーを落として帰ると、翌日出社して電気をつけたときには代表電話番号が昨日とは違うものに変わっている、というような感じです。毎日「弊社の電話番号が変わりまして。今日の番号はこれです」とクライアントへ連絡するのは嫌です。
...
//image[startaws5][「割り当て」をクリック][scale=0.8]{
//}

//image[startaws6][「閉じる」をクリック][scale=0.8]{
//}
...
//footnote[bastion][今回はサーバが1台だけなので直接ログインしていますが、通常は踏み台サーバ（英語だとBastion hostと呼ばれることが多い）を用意して外部からは踏み台サーバにしかログインできないようにしておき、個々のウェブサーバやDBサーバには踏み台を経由しないと入れない、という構成にすることが多いです。サーバは大切な箱入り娘なので、怖いお父さん（踏み台サーバ）を介さないと話しかけられないようにすることで安全性を高めているのです。]

でも折角つけたElasticIPですが、この「52.199.234.148」のような数字の羅列を覚えたり、SSHでログインするたびに打ち込むのはちょっと面倒ですよね。

皆さんも普段電話をかけるときに、電話番号をいちいち打つのは面倒なのでアドレス帳に名前と電話番号を登録していますよね。それと同じようにネームサーバにドメイン名とIPの紐づけを登録しておけば、ドメイン名を使ってサーバに入れるようになります。

皆さんは「DNSをはじめよう」で買った自分のドメインを持っているので、早速ドメイン名とIPアドレスの組み合わせをネームサーバに登録してみましょう。ドメイン名の設定はRoute53で行います。
...
Domain Nameの自分のドメイン名（私の場合はstartdns.fun）をクリック（@<img>{startaws17}）します。
...
Macの方は次のコマンドを実行してください。「キーボードで「↑」を押すと直前に打ったコマンドが出てきますので、アットマークの後ろを「login.自分のドメイン名」に変更してReturnキーを押してください。
...
と書いてEnterキーを押すとrootになれます。（@<img>{startaws28}）また何回かEnterキーを押して改行してみましょう。いちばん左側に出ているプロンプトが次のように変化しましたか？
...
ユーザ名が「ec2-user」から「root」に変わりました。それからいちばん右の部分も「$」から「#」に変わっています。プロンプトは一般ユーザだと「$」、全権を持っているrootだと「#」という表示です。今後は「このコマンドをrootで実行してください」のように実行ユーザを詳しく書くことはしませんので、書いてあるプロンプトが「$」だったらec2-userのような一般ユーザで実行、「#」だったらrootで実行するんだ、と思ってください。ターミナル上で「$」や「#」を自分で入力する必要はありません。
...
このように何か失敗したときだけエラーが出ます。英語でエラーが出るとそれだけでパニックになってしまいますが、落ち着いてゆっくり読めば「sudooo: command not found・・・ああ、sudoooっていうコマンドが見つかりませんでした、って書いてある」と判読できると思います。エラーが出たら声に出してゆっくり読んでみましょう。
...
コピーした内容をターミナルへペーストしたいときは@<ttb>{右クリックするだけ}です。Ctrl+Cは使えないので注意してください。
...
先ずはyumで色々アップデートしておきましょう。Windowsアップデートみたいなものです。最後に「Complete!」と表示されたら問題なく完了しています。

//cmd{
# yum update -y
//}

続いて「DNSをはじめよう」で出てきたwhoisコマンドと、DB接続時に必要なmysqlクライアントを入れておきます。こちらも最後に「Complete!」と表示されればOKです。

//cmd{
# yum install -y jwhois mysql
//}

さらにPHPとApacheも入れます。PHPはApacheのモジュール（部品）なので、PHP7をインストールしようとするとyumが気を利かせてApache2.4系も入れてくれます。
...
この状態でi（アイ）を押すと「編集モード」に変わります。（@<img>{startaws32}）左下に「-- INSERT --」と表示されていたら「編集モード」です。
...
「閲覧モード」に戻ったら「:wq」@<fn>{wq}と入力してEnterキーを押せば保存されます。（@<img>{startaws35}）

//image[startaws35][「:wq」と入力してEnterキーを押せば保存される][scale=0.8]{
//}

//footnote[wq][write（書き込んで）、抜ける（quit）という命令なのでwqです。]
...
//footnote[cat][catは猫ではなく「conCATenate files and print on the standard output」の略だそうです。私はいつも「猫がファイルの中身を全部出して見せてくれてるんだ」と考えることでちょっぴり幸せになっています。]

さらにlnコマンドでシンボリックリンクを作ります。シンボリックリンクはWindowsでいうところのショートカットみたいなものですね。
...
//image[startaws36][「:wq」と入力してEnterキーを押せば保存される][scale=0.8]{
//}
...
書き終わったらESCキーを押して「閲覧モード」に戻り「:wq」で保存します。編集できたらcat（キャット）コマンド@<fn>{cat}でファイルの中身を確認してみましょう。
...
//footnote[tail][tailコマンドは名前のとおりファイルの尻尾、つまり末尾を表示するコマンド。引数で「-2」と指定すれば末尾から2行、「-10」と指定すれば末尾から10行が表示される。]
...
設定変更してrebootするまでは日時が記録されていなかったためすべて同じ日時になっていますが、reboot後はちゃんと実行した日時が表示されています。ちなみにhistoryは「exit」するときに履歴が書き込まれるため、前述の「exit」で抜けずに赤い×を押してウィンドウを閉じたりするとその分は記録されずに消えてしまいます。
...
サーバも同じです。サーバを使うときは「ターミナル」という操作するための画面を開いて、ディレクトリ@<fn>{dir}を開いて移動するときはダブルクリックの代わりにcd@<fn>{cd}」というコマンドを叩いて移動しますし、ディレクトリの中を見るときはもダブルクリックの代わりにlsコマンドを叩いて見ます。
