...
「インスタンスの作成」をクリックすると、作成ステータスの画面で「インスタンスは現在作成中です」（@<img>{createStatus}）と表示されます。インスタンスができあがるまでの間に請求アラートを作っておきましょう。「請求アラートの作成」をクリックしてください。

//image[createStatus][「インスタンスは現在作成中です」と表示されたら「請求アラートの作成」をクリック][scale=0.8]{
//}

=== 現代のパケ死を免れるための請求アラート

AWSはどのサービスも従量課金です。そのため誤ってすごく高スペックなサーバを立ててしまって1か月後に青ざめる・・・とならないよう、利用額が一定金額を超えたらメールでアラートを飛ばすことができます。

「電子メールで PDF 版請求書を受け取る」と「請求アラートを受け取る」にチェックを入れて、
「設定の保存」ボタンを押します。

こうしたアラームは、AWSのCloudWatchというサービスで行います。

上に一瞬緑色の「設定を保存しました」という表示が出たら、次は「請求アラートを管理する」リンクを押して、CloudWatchのダッシュボードに遷移します。

「請求アラームを作成すると」リンクを押して、請求アラームを作成します。

超過、という欄に1USDと書けば、1ドルを超えた時点でアラートが来ます。今回は無料利用枠からはみ出す分が最大でも月2ドルくらいなので、2と書いておきましょう。

通知の送信先は、自分のメールアドレスにしておきましょう。これで、請求額が閾値を超えると、すぐに請求アラームがメールで届きます。
これ設定しておかないと、万が一アカウントが盗まれて、大量のインスタンスを立てられたときにも月末まで気付かずに後からすごい請求が来るのでご注意ください。

超過額と、送信先のアドレスを書いたら「アラームの作成」ボタンを押します。

新しいメールアドレスを登録した場合、「AWS Notification - Subscription Confirmation」という件名でメールアドレスの確認メールが届くので、メール本文中のConfirm subscriptionというリンクを踏んでください。

Subscription confirmed!と表示されることを確認したら、この画面は閉じて構いません。先ほどの画面に戻ります。

すると元の画面でメールアドレスの横に緑のチェックマークがつくので、「アラームの表示」ボタンを押してください。

緑色で「アラーム BillingAlarm は保存されました。」と表示されるので、中央部のBillingAlarmのグラフをクリックしてください。BillingAlarmの状態がOKになったら、請求アラートの設定は完了です。

オレンジ色で「データ不足」と出る場合は、右上のぐるぐるした更新マークを押してみてください。状態がOKになりましたでしょうか？

これで、請求額が指定の閾値を超えると、「ALARM: "BillingAlarm"」という件名のメールが届くようになります。

では請求アラートを設定している間に、インスタンスも出来上がっているものと思います。いよいよサーバにSSHで接続してみましょう。
