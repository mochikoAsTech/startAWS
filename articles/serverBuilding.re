...
それではいよいよサーバに入りましょう。続いてターミナルで次の文字を入力したら再びReturnキーを押します。「IPv4パブリックIP」の部分は、先ほどメモした「IPv4パブリックIP」に書き換えてください。-iオプションは「この鍵を使って入ります」という意味ですので、もし「start-aws-keypair.pem」を保存した場所がデスクトップ以外の場合は適宜書き換えてください。

//cmd{
ssh ec2-user@IPv4パブリックIP -i start-aws-keypair.pem
//}

初回のみ次のようなメッセージが表示されますが、これは「初めて入るサーバだけど信頼していいですか？本当に接続しますか？」と聞かれていますので、「yes」と打ってReturnキーを押してください。するとターミナルが覚えていてくれて、次回以降は「これは前に信頼していいって言われたサーバだ！」と判断してそのまま接続させてくれます。
...
と打ってEnterキー（もしくはReturnキー）を叩いてみましょう。これはサーバをシャットダウンするコマンドです。シャットダウンが気軽に試せるのは勉強用のインスタンスならではですね。

打ってしばらくするとサーバが無事にシャットダウンされて、SSHの接続も切れてしまいました。それではちゃんとシャットダウンできたのか、EC2のダッシュボードからも確認してみましょう。
