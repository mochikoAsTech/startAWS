...
無事にEC2でインスタンスが作成できたので、いよいよサーバに入ってみましょう。

=== サーバに「入る」とは？

ところでサーバに「入る」といわれてもピンと来ない方もいるのではないでしょうか。サーバに「入る」って、いったいどういうことでしょう？
...
パスワード認証ではユーザ名とパスワードを使ってサーバに入ります。鍵認証の場合はあらかじめサーバに鍵穴（公開鍵）を設置しておいて、パソコンの中にある鍵（秘密鍵）を使ってサーバに入ります。この鍵というのが、皆さんが先ほどダウンロードしたキーペア（start-aws-keypair.pem）のことなのです。
