...
このときマンションの建物にあたる物理サーバを@<ttb>{ホストサーバ}、101号室や201号室のような各部屋にあたる仮想サーバを@<ttb>{ゲストサーバ}と呼んだりします。
...
昔々は、企業が「そろそろ自社のウェブサイト作りたいなぁ・・・サーバが欲しい！」と思ったら、「どのメーカーのサーバにしよう？EHPかな？それともIBMかな？DELLがいいかな？」と各社の見積もりをとって、値引き交渉をして、それでも数十万から数百万するので社内の稟議を通してやっと購入。購入してもすぐ届くわけではなく、数週間待ってやっと届きます。そして届いたら段ボールから出して、データセンターもしくは自社のサーバルームのサーバラックまで持って行って、がっちゃんこと設置したら今度は同じく自前のネットワーク機器からLANケーブルを繋いで電源も繋いで、OSのインストールディスクを用意したらサーバにOSをインストールして・・・以下省略しますが、要は自分でサーバを買って、何もかも自分で用意しないといけませんでした。
...
クラウドなら「自社のウェブサイト作りたいなぁ」と思ったら、サイト上で使いたいサーバのスペックを選んでぽちっとなするだけで、すぐにサーバを立てることができます。AWSなら課金も1秒単位の従量課金なので、たとえばサーバを5分使ったら5分ぶんの費用しかかかりません。

オンプレミスはサーバを買って使う、クラウドはサービスとして使う、ということです。でもまだちょっとわかりにくいと思うので、お店を例にしてみましょう。

==== クラウドのメリット

たとえば私が突然ピザ作りに目覚めて、もうインフラエンジニアなんかやってる場合じゃない！ピザ屋を始めるんだ！@<fn>{pizza}と思い立ったとします。

//footnote[pizza][そしたら「ピザ屋をはじめよう」という本が書けますね。]

ピザ屋さんをオープンすべく、土地を買って、そこに店舗となる建物を建てて、電気とガスと水道を通して、床板とか壁紙とか貼って・・・からやると、お金も場所も時間もたくさん必要送ですよね。しかも準備が整ってやっとオープンしたと思ったら、たった1か月で資金が足りなくなってお店がつぶれることになったとしても、今度は建物の取り壊しとか土地の処分とか、それはそれでやることがたくさんあります。このように全部自分で買って、自分で所有・管理するオンプレミスだと、「ちょっと気軽にピザ屋さんをやってみよう」はなかなか厳しいことが分かります。

一方クラウドは、フードコートへの出店に似ています。「ピザ屋をはじめたい！フードコートの一区画を借りてやってみよう！」という感じです。

これだと建物はもうあって、電気ガス水道ももう準備されています。フードコート内の一区画を契約して使わせてもらうだけなので、すべて自分で準備するオンプレミスと違ってすぐに始められます。しかも数か月やってみて「もうピザ焼くの飽きたー！」と思ったら、その区画を借りるのを止めるだけでいいのです。前述のとおりAWSなら1秒単位の従量課金なので「前月の25日までに契約終了を申し出る必要がある」や「フードコード内の区画を借りる契約は1年単位」といった制限がありません。いつでもやめられます。
...
いつ来るか分からない地震に備えて、最初から2か所にお店を構えておくとなったら、相当なコストがかかります。サーバでいったら、ただ自社サイト作りたいだけなのに、東京と大阪の2か所にデータセンターを用意して両方に1台ずつサーバ置いとかないといけない、みたいな感じです。これを勝手にやってくれるクラウドすごい。
...
AWSでも大規模障害は定期的に起きています。去年も豪雨による電源障害でサーバがダウンしたりしてました。こうした障害の際も、AWSが発表してくれることがすべてなので、原因が分かるまで自分で徹底的に調べる、ということができません。
...
いっぱい書いたのでので、もう1回おさらいすると、「自社サイト作りたいなぁ」と思ったら自分でサーバを買わないといけないのがオンプレミスで、「自社サイト作りたいなぁ」と思ったらサーバを従量課金ですぐ使えるのがクラウドです。

そしてようやく最初の話に戻ると、AWSは、Amazonがやっているクラウドのことです。
