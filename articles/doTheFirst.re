...
AWSはどのサービスも従量課金です。誤って高スペックで高額なサーバを立ててしまい1か月後に青ざめる・・・ということにならないよう、利用額が一定金額を超えたらメールでアラートを飛ばす設定をしておきましょう。こうしたアラートの設定はAWSのCloudWatchというモニタリングサービスで行います。
...
アラームの作成画面（@<img>{credits06}）で「超過」という欄に1USDと書くと、今月の利用料が1ドルを超えた時点でアラートメールが来ます。本著の内容でAWSを利用した場合、無料利用枠からはみ出す分が最大でも月2ドル程度ですのでここには2と書いておきます。「通知の送信先」には自分のメールアドレスを記入してください。
...
すると元の画面でメールアドレスの横に緑のチェックマークがつく（@<img>{credits10}）ので、「アラームの表示」をクリックしてください。
...
BillingAlarmの状態がOK（@<img>{credits12}）になっていたら請求アラートの設定は完了です。もしオレンジ色で「データ不足」と表示されていた場合は、右上の更新マークをクリックしてみてください。
...
さて、皆さんがいま使っているのは「ルートユーザー」と呼ばれるユーザです。実はルートユーザーは全権を持っていてなんでもできるユーザなので、普段からこのユーザを使って色々な操作をするのはお勧めしません。
