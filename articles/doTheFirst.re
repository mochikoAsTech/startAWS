...
ちなみに「DNSをはじめよう」は、ただのDNS好きである筆者がDNSへのあふれんばかりの愛を早口で詰め込んだ本ですが、技術書典4当日に750冊、その後もダウンロード販売で売れ続けて累計1300冊以上（2018年8月現在）という驚きの頒布数@<fn>{journal}となりました。技術季報手にとって、買って、読んでくださった皆さん、ありがとうございます。
...
それでは早速、AWSのサインイン画面（@<href>{https://console.aws.amazon.com/}）を開いて（@<img>{consoleSignin}）、マネジメントコンソールにサインインしましょう。サインインという言葉には馴染みがないかも知れませんが、ログインと同じ意味です。
