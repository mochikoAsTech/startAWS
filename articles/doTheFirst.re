...
この章ではAWSの管理画面にサインインして、最初に行うべき設定をします。
...
AWSのユーザにはクレジットカードを紐付けているのですから、お財布もルートユーザーも等しく扱いには気をつけなければいけません。たとえば悪い人があなたのルートユーザのEメールアドレスとパスワードを盗んで、こっそりマネジメントコンソールにサインインしたとします。ルートユーザーならなんでもできるので、景気良くいちばん高いサーバ@<fn>{d28xlarge}を100台立てた@<fn>{reasonWhy}としましょう。その場合、1日で180万なので、1ヶ月後には5,400万の請求があなたのところへやってきます。（@<img>{awsUnauthorizedUse}）
...
//footnote[reasonWhy][悪い人たちは他人のアカウントで高スペックのサーバを大量に立てて、ビットコインを生み出すためのマイニング（採掘）をするのです。]
...
本著では1人だけで作業をする想定なので、IAMユーザーも1人しか作りません。ですがもし業務などで複数名でマネジメントコンソールにサインインするような場合は、IAMユーザーは必ず1人につき1つずつ作成してください。まったく同じ作業をするから開発チーム内のAさんとBさんは同じIAMユーザを共有すればいいのでは？と思われる場合でも、必ずAさんBさんそれぞれに別々のIAMユーザーを用意することをお勧めします。
...
「IAM」をクリックすると、IAMのダッシュボード（@<img>{iamDashboard}）が表示されます。IAMはIdentity and Access Managementの略で、AWSの利用を安全に管理するためのサービスです。
...
//footnote[administratorAccess][え、大金の詰まったアタッシュケース持って買い物に行っちゃうの？！と思ったあなたは正しいです。ですが、ここでは「普段はルートユーザーではなくIAMユーザーを使うべき」「本来はIAMユーザーには必要最小限の権限だけを付与すべき」ということだけ覚えておいて先へ進みましょう。]
...
IAMのグループが作成できたので、続いてIAMユーザーを作成しましょう。左メニューから「ユーザー」を選ぶと、ユーザーの一覧画面（@<img>{selectUser}）が表示されます。まだIAMユーザーが1つも存在しないため、「IAMユーザーが存在しません。」と表示されています。それでは左上の青い「ユーザーを追加」をクリックしてください。
...
続いてこのIAMユーザーでAWSにアクセスする方法を選択します。たとえばAWSで「サーバを立てる」「サーバを削除する」などの操作をするには、
...
最後にステップ3です。次の4つを確認して、問題なければ「ユーザーの作成」をクリック（@<img>{iamStep3}）してください。
  ...
  * AWSアクセスの種類が「AWS マネジメントコンソールへのアクセス - パスワードを使用」であること
...
IAMユーザー情報	例	自分のIAMユーザー情報をここにメモ
------------------------------------
...
しっかりメモをしてメールも送ったら、画面の「https://************.signin.aws.amazon.com/console」と書いてあるURLをクリックします。するとブラウザの別タブでIAMユーザーのサインイン画面が開いて、自動的に元のタブのルートユーザーはサインアウト（@<img>{signOut}）させられます。サインアウトしてしまったタブは閉じてしまって構いません。
...
サインインすると、新しいパスワードの設定画面（@<img>{changePassword}）が表示されます。「古いパスワード」に先ほどメモしたパスワードを、「新しいパスワード」には自分で考えたパスワードを入力してください。すべて入力したら「パスワード変更の確認」をクリックします。
...
右上のIAMユーザー名をクリック（@<img>{checkUsername}）すると、IAMユーザー名とAWSアカウントが表示されるので、ルートユーザーではなくIAMユーザーでサインインしていることが確認できます。
...
3つがあれば、IAMユーザーでサインインできてしまいます。ですがMFA（多要素認証）@<fn>{mfa}を有効にすると「AWSアカウントとユーザー名とパスワード」に加えて、ユーザーが持っているスマホの認証アプリなど別の要素を使って本人か確認することになるため、より安全にアカウントを管理できます。
...
スマホで「Google認証システム」のアプリを起動したら右下の赤い＋ボタンをタッチ（@<img>{mfaAppli01}）して、「バーコードをスキャン」を選択（@<img>{mfaAppli02}）します。
...
「赤線内にバーコードを配置してください」（@<img>{mfaAppli03}）と表示されるので、マネジメントコンソールに表示されたバーコード（@<img>{barcode}）がスマホの赤枠内に収まるようにします。
...
右上に表示されている「リージョン」とはその各地域の中でどこを使うか？を指定するものです。ウェブサイトにアクセスするとき、パソコンのある場所からサーバまで物理的に距離が遠いと、それだけ通信にも時間がかかって応答時間も遅くなりますので、日本国内向けにウェブサイトを開設する場合は基本的にこの「東京」リージョンを選びましょう。ただしAWSのサービスによって、まだ東京リージョンが使えないものもあります。その場合は次点として「米国東部 (バージニア北部)」を選択してください。
...
//footnote[addCloudTrail][業務で使うので90日よりももっと前の記録も取っておきたい、という場合は「証跡（しょうせき）の作成」を行ってください。]
