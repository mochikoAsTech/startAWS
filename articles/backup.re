...
@<chapref>{serverBuilding}ではEC2で立てたインスタンスにログインして色々な設定をしました。@<chapref>{apacheSetting}ではバーチャルホストを作りました。@<chapref>{wordPress}ではWordPressのインストールをしました。インスタンスのOSやviで頑張って修正した設定ファイル、ドキュメントルートに設置したWordPressのファイルなどはすべてEBSボリューム@<fn>{ebsStorage}に保存されています。ある日うっかり「rm -rf /etc/」@<fn>{rm}を叩いてしまって「折角設定したファイルが全部消えちゃった・・・バックアップもない・・・」と泣くことのないよう、EBSボリューム（つまりハードディスクに保存されたデータ）のバックアップを取っておきましょう。
